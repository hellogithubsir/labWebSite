...
test("team screen member directory and filtering", async ({ page }) => {
  ...
  await expect(cards.locator("h3")).toHaveText(["Zhao Yanfeng", "Cheng Xiang", "Liu Jianbang", "Dr. Jeff Wang", "Muhammad Aiman Md Zuki", "Leong Pooi Yan", "Zheng Kun", "Ooi Tze Yaang", "Teo Shi Han"]);
  await page.getByRole("button", { name: "Graduates", exact: true }).click();
  await expect(cards.locator("h3")).toHaveText(["Zhao Yanfeng", "Cheng Xiang", "Liu Jianbang", "Dr. Jeff Wang", "Zheng Kun", "Ooi Tze Yaang", "Teo Shi Han"]);
  await expect(page.getByRole("button", { name: "Graduates", exact: true })).toHaveAttribute("aria-pressed", "true");
});

const names = ["Zhao Yanfeng", "Cheng Xiang", "Liu Jianbang", "Dr. Jeff Wang", "Muhammad Aiman Md Zuki", "Leong Pooi Yan", "Zheng Kun", "Ooi Tze Yaang", "Teo Shi Han"];
const bios = {
  "en": [
    ...
    "Academic publishing name: Wang Hongqing. Develops lightweight object detection models for complex traffic scenes.",
  ...
  "zh": [
    ...
    "研究多模态情绪分析，支持个性化人机交互。",
    "学术署名：Wang Hongqing。开发适用于复杂交通场景的轻量化目标检测模型。",
...
for (const width of [1920, 390, 320]) for (const chinese of [false, true]) {
 test(`team screen ${width} ${chinese ? "Chinese" : "English"} content media filters`, async ({ page }, testInfo) => {
  ...
  await expect(cards.locator("h3")).toHaveText(chinese ? ["Zhao Yanfeng", "Cheng Xiang", "Liu Jianbang", "王泓清博士（Jeff Wang）", "Muhammad Aiman Md Zuki", "Leong Pooi Yan", "Zheng Kun", "Ooi Tze Yaang", "Teo Shi Han"] : names);
  const roles = chinese ? ["博士毕业生 / 核心成员","博士研究生","硕士毕业生"] : ["PhD Graduate / Core Member","PhD Candidate","MSc Graduate"];
  for (let index=0; index<9; index++) {
    await expect(cards.nth(index).locator("p").nth(0)).toHaveText(index === 3 ? (chinese ? "博士毕业生" : "PhD Graduate") : roles[Math.floor(index/3)]);
  ...
  for (const [filterIndex, expected] of [[1,["Zhao Yanfeng","Cheng Xiang","Liu Jianbang","Dr. Jeff Wang","Zheng Kun","Ooi Tze Yaang","Teo Shi Han"]],[2,["Muhammad Aiman Md Zuki","Leong Pooi Yan"]],[0,names]] as const) {
   await buttons.nth(filterIndex).scrollIntoViewIfNeeded(); await buttons.nth(filterIndex).focus(); await page.keyboard.press("Enter");
   await expect(cards.locator("h3")).toHaveText(expected.map(name => chinese && name === "Dr. Jeff Wang" ? "王泓清博士（Jeff Wang）" : name));
  ...
  await expect(cards.locator("h3")).toHaveText(chinese ? ["Muhammad Aiman Md Zuki","Leong Pooi Yan"] : ["Muhammad Aiman Md Zuki","Leong Pooi Yan"]);
