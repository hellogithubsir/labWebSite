...
export function ContactScreen({ locale }: { locale: Locale }) {
  ...
  return <div className={styles.screen}>
    ...
    <section className={styles.targets} aria-labelledby="contact-title-1" data-od-id="contact-targets"><div className={styles.container}>
      <ScrollReveal>{heading(1)}</ScrollReveal><div className={styles.targetGrid}>{copy.targets.map((target, index) => <ScrollReveal key={target.title}><article><b>C-02-0{index + 1}</b><h3>{target.title}</h3><dl>{target.fields.map(field => <div key={field.label}><dt>{field.label}</dt><dd>{field.body}</dd></div>)}</dl><p className={styles.note}>{target.note}</p></article></ScrollReveal>)}</div>
    </div></section>
