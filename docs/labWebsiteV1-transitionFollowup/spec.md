...
| 完成 | 1500ms | 清锁，恢复目标画面可访问性和交互，并按既有焦点约定把焦点落到目标画面标题。 |
...
先用现有 transition 套件记录旧实现相对新时序断言的失败，再修复并记录通过；测试和说明应覆盖桌面轨道、移动内容、同屏、重复输入、URL、Locale、单可访问画面及焦点。完整 release gate 的持久 trace 和命令日志放在 `docs/design-references/hil-site/transition/`，trace 输出目录和日志目录分开，避免 Playwright 清理自身日志。
...
- VAL-MOTION-003: 单URL与交互状态, Behavior: 七画面保持单URL及Locale；首目标锁定且同屏不重置；键盘和移动菜单可用；过渡中至多一个可访问画面且完成后目标标题焦点正确, Surface: ui, Evidence: 双语七屏导航与同屏及重复输入和可访问性焦点断言报告及trace
