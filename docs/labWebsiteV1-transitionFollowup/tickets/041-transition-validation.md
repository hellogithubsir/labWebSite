...
不构建产品、不修改被测实现。读取 spec.md、040最终差异及证据，对照原站现场观察和本项目适配要求。验证轨道从700ms才动、内容与页脚完整1500ms时序、轨道几何与移动断点、单URL和Locale、同屏/输入锁、至多一个可访问画面、按来源恢复焦点（桌面目标rail、移动菜单按钮、正文CTA的site-content或等价主标题，同屏不移动，reduce同规则）和reduce100ms内结束。

独立运行实际测试，审阅完整make check-release的新证据；按协议在新mission被Git忽略的 missions/evidence/ 保存逐条验证报告、trace、截图和完整日志；tracked docs仅放文本索引/JSON并链接持久证据，不放可执行临时脚本或二进制。不得将旧030/031或旧250ms通过记录视为本轮通过。缺失证据记blocked，真实失败记fail，交还实现worker修复；validator不自行修被测代码。
